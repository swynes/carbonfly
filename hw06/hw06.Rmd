...
late_sing_loc %>% 
  na.omit() %>% 
  revgeocode(location=c(late_sing_loc$longitude,late_sing_loc$latitude), output=c("more")) #This doesn't work because revgeocode wants a vector and not a column in a dataframe, I guess. Will try and use possible() to make it happen
...
late_sing_loc %>% 
  na.omit() %>% 
sapply(11:12, revgeocode(location=c(late_sing_loc$longitude,late_sing_loc$latitude), output=c("more")), simplify = TRUE)


library(gapminder)
sapply(gapminder$lifeExp, mean, simplify = TRUE)
